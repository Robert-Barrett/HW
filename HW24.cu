// Name: Robert Barrett 
// nBody run on all available GPUs using CUDA Unified Memory
// nvcc HW24.cu -o temp -lglut -lm -lGLU -lGL

/*
 Modified to use CUDA Unified Memory and cudaMemPrefetchAsync for simpler
 memory management and faster memory movement between GPUs.
*/
...
// Globals - Now using single unified memory pointers instead of per-GPU arrays
int N;
int NPerGPU;
int NumberOfGpus;
float3 *P, *V, *F;  // Now allocated with cudaMallocManaged
float *M; 
...
void drawPicture()
{
	...
	// With unified memory, P is already accessible from host - no explicit copy needed
	// But we prefetch to CPU to ensure data is available
...
void setup()
{
	...
	GridSize.x = (NPerGPU - 1)/BlockSize.x + 1;
	GridSize.y = 1;
	GridSize.z = 1;
	
    	Damp = 0.5;
    	
    	// Allocate unified memory - accessible from both CPU and all GPUs
    	int nn = NumberOfGpus*NPerGPU;
	...
	// Initialize data on CPU - unified memory makes this straightforward
	for(int i = 0; i < N; i++)
	{
		test = 0;
		while(test == 0)
	...
	// Prefetch data to all GPUs for better initial performance
	for(int i = 0; i < NumberOfGpus; i++)
	{
		cudaSetDevice(i);
		cudaMemPrefetchAsync(P, nn*sizeof(float3), i);
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemPrefetchAsync(V, nn*sizeof(float3), i);
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemPrefetchAsync(F, nn*sizeof(float3), i);
		cudaErrorCheck(__FILE__, __LINE__);
		cudaMemPrefetchAsync(M, nn*sizeof(float), i);
		cudaErrorCheck(__FILE__, __LINE__);
	}
...
void nBody()
{
	...
	while(t < RUN_TIME)
	{
		// Prefetch position data to each GPU before computation
		for(int i = 0; i < NumberOfGpus; i++)
		{
			cudaSetDevice(i);
		...
		// Compute forces and move bodies on each GPU
		for(int i = 0; i < NumberOfGpus; i++)
    		{
			cudaSetDevice(i);
		...
		// Synchronize all GPUs - unified memory handles data consistency automatically
		for(int i = 0; i < NumberOfGpus; i++)
    		{
			cudaSetDevice(i);
			cudaDeviceSynchronize();
