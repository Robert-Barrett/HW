...
{
    // Shared memory for reduction - size of block
    __shared__ float shared_data[1000];
    
    // Get thread ID
    int tid = threadIdx.x;
    int idx = tid; // Since we only have one block, idx = tid
    
    // Each thread computes one element-wise multiplication if within bounds
    if (idx < n) {
    ...
    // Reduction using folding at block level
